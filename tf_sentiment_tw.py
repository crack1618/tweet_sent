...
class Cnn:
  ...
  def def_params(self):
    ...
    with tf.name_scope('params'):
      ...
      with tf.name_scope('fc1'):
        self.W_fc1 = self.weight_variable([7 * 7 * 256, 1024])
  ...
  def def_model(self):
    ...
    with tf.name_scope('conv1'):
      h_cn1 = tf.nn.relu(self.conv2d(X, W_cn1) + b_cn1)
    ...
    with tf.name_scope('fc1'):
      h_pool1_flat = tf.reshape(h_pool1, [-1, 7 * 7 * 256])
  ...
  def train(self, data):
    ...
    with tf.Session() as sess:
      ...
      train_writer = tf.summary.FileWriter('graphs/ex9_cnn_train')
      test_writer = tf.summary.FileWriter('graphs/ex9_cnn_test')
